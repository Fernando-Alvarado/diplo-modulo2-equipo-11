...
knitr::kable(head(aeropuertos), caption = "Aeropuertos")
knitr::kable(head(aerolineas), caption = "Aerolíneas")
knitr::kable(head(aviones), caption = "Aviones")
knitr::kable(head(clima), caption = "Clima por hora")
knitr::kable(head(vuelos), caption = "Vuelos salientes")
...
vuelos |> 
  left_join(clima, by = "time_hour") |>
  summarise(correlacion = cor(wind_speed, dep_delay, use = "complete.obs"))

```

#### **¿Cuál es la correlación entre la velocidad del viento y los retrasos en la salida?**

Se identifica una **correlación baja entre la velocidad del viento y los retrasos en la salida de los vuelos**, con un coeficiente de **0.054**. Este valor indica una relación prácticamente nula, lo cual sugiere que, en condiciones normales, **la velocidad del viento no afecta significativamente las operaciones aéreas**. Es probable que solo **condiciones extremas**, como vientos huracanados o tormentas severas, puedan generar impactos reales en los procesos de **despegue o aterrizaje**.

```{r, echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
# ¿Los vuelos experimentan más retrasos en días con lluvias intensas?
vuelos |> 
  left_join(clima, by = "time_hour") |>
  summarise(correlacion = cor(precip, dep_delay, use = "complete.obs"))
```
#### **¿Los vuelos experimentan más retrasos en días con lluvias intensas?**

Se observa que la **correlación entre los retrasos en los vuelos y la cantidad de lluvia es de 0.085**, lo que indica una **relación muy débil** entre ambas variables. Este resultado sugiere que, en condiciones normales, **la lluvia no representa un factor determinante** en los retrasos. Para que el clima tenga un impacto significativo, **serían necesarias condiciones extremas**, como **tormentas severas o huracanes**, que efectivamente puedan impedir las operaciones de despegue o aterrizaje de las aeronaves.

```{r, echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
# ¿Cómo afecta la temperatura a los retrasos de los vuelos?
vuelos |> 
  left_join(clima, by = "time_hour") |>
  summarise(correlacion = cor(temp, dep_delay, use = "complete.obs"))
```

#### **¿Cómo afecta la temperatura a los retrasos de los vuelos?**


De manera similar, **no se observa una correlación significativa entre la temperatura y los retrasos en los vuelos**, ya que el coeficiente de correlación es de apenas **0.0601**. Este valor indica una **relación prácticamente nula**, lo que sugiere que la **temperatura ambiental no influye de forma directa** en los retrasos registrados. Por tanto, puede descartarse como un factor determinante en este contexto.
...
vuelos |> 
  left_join(clima, by = "time_hour") |>
  summarise(correlacion = cor(visib, arr_delay, use = "complete.obs"))
```

#### **¿Cómo afectan los niveles de visibilidad a los retrasos en la llegada?**

En este caso se observa una **correlación negativa de -0.131**, ligeramente mayor en magnitud que en el análisis anterior, aunque sigue sin ser estadísticamente significativa. Este resultado sugiere que, si bien la **visibilidad** puede tener algún efecto sobre los **retrasos en la llegada de los vuelos**, dicho efecto es **limitado**.

Esto puede explicarse porque los aeropuertos modernos operan con **sistemas tecnológicos avanzados de navegación y aterrizaje**, que **no dependen exclusivamente de la visibilidad** directa del piloto. Por lo tanto, la baja visibilidad podría contribuir en cierta medida a los retrasos, pero **no constituye una causa principal**.
...
vuelos |> 
  left_join(clima, by = "time_hour") |>
  summarise(correlacion = cor(humid, dep_delay, use = "complete.obs"))
```
#### **¿La alta humedad se relaciona de alguna manera con los tiempos de taxi-out más largos?**

La correlación entre la **humedad** y el **retraso en la llegada de los vuelos** no resulta significativa, ya que el coeficiente obtenido es de apenas **0.116**. Este valor sugiere una **relación muy débil** entre ambas variables, lo cual permite concluir que, en este caso, **la humedad ambiental no tiene un efecto relevante en los retrasos de los vuelos**.
